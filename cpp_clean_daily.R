...
#### (Change) Change the location/title of the original file ####
##test <- read.csv("~/Desktop/ofe/cpp_jan.csv", head=TRUE,fileEncoding="latin1")
names(test) <- c("VESSEL","CARGO_SIZE","VESSEL_TYPE","CARGO","LOAD_PORT","LOAD_COUNTRY","LOAD_ZONE","DISCHARGE_PORT","DISCHARGE_COUNTRY","DISCHARGE_ZONE","LAYCAN","RATE","CHARTERER","STATUS","UPDATE_DATE", "UPDATE_MONTH","SOURCE")
noname <- which(test$VESSEL == "")
if (length(noname)>0) test<- test[-noname,]
test <- unique(test)

# (Change) Change the location of the full vessel list file ####
...
for (i in 1:ncol(test)){
  ...
  test[,i] <- gsub("\312|_","",test[,i])
  test[,i] <- gsub("#REF!","", test[,i])
...
s_detect <- c("FLD|FAILED| F$|DENIED", "HOLD", "SUB|/S| S$", "RPLC|REPLACE|REPL|RPL", "/COR| COR$|COR\\)|CORR$|CORR\\)|/CORR|\\*|CORRECT|UPDT|RATE ADD|UPDATE", " OLD", "OOS|O/O|OO$", 
              "REFXD|FXD|FIXED| C$|CFMD|CONFIRM|CONF$|^FX$", "N/B", "CNCL|CANC|CANCLD", " DD|EXDD|EX D/D|D/D", "SWAPPED")
status_corr <- c("FLD", "HOLD", "SUB", "RPLC", "CORR", "OLD", "OOS", "FXD", "N/B", "CNCL", "DD", "SWAPPED")
status <- ""
e_stat <- which(test$SOURCE == "E")
test$STATUS[e_stat] <- gsub("- FLD.*|- RLSD.*|- REPLACED.*", " RPLC", test$STATUS[e_stat])
...
status[which(is.na(status))] <- "SUB"
test$STATUS <- status
test_c <- test

# Clean CARGO_SIZE-------------------------
if(sum(is.na(test$CARGO_SIZE))>0) test <- test[-which(is.na(test$CARGO_SIZE)),]
for (i in 1:nrow(test)){
  k <- as.numeric(str_extract(test$CARGO_SIZE[i], "[0-9]{1,3}"))
  if (str_detect(test$CARGO_SIZE[i], "K")| k > 150) test$CARGO_SIZE[i] <- round(k/7.33)
  else test$CARGO_SIZE[i] <- k
}
test$CARGO_SIZE[which(!str_detect(test$CARGO_SIZE,"[0-9]"))] <- 0
test_c <- test

# Clean CARGO -------------------
test$CARGO <- str_trim(test$CARGO)
test$CARGO <- gsub("\\?","", test$CARGO)
test$CARGO[which(is.na(test$CARGO))] <- "CPP"
...
test$VESSEL <- gsub(" OOS.*$|O/O.*$|\\-|\\?","",test$VESSEL) # Delete OOS and O/O
test$VESSEL <- gsub("/.*|\\(.*", "", test$VESSEL)  # Delete /... and (...)
test$VESSEL <- gsub(" COR$| CORR$| SUB$| OLD$| FLD$| HOLD$| P$| PC$| OO$| DD","", test$VESSEL) 
test$VESSEL <- gsub("MT |MV |OOS$|RPLC|LR2$|\\||\\[|^M\\.T\\.|^M\\.V\\.","", test$VESSEL) #delete "MT""MV"
test$VESSEL <- gsub("\\s+"," ", test$VESSEL)
test$VESSEL <- str_trim(test$VESSEL)
if(sum(test$VESSEL==""|test$VESSEL == "VESSEL")>0) test <- test[-which(test$VESSEL==""|test$VESSEL == "VESSEL"),]
...
test$CHARTERER <- gsub('16 FLAT\\)| SUBS|2nd|2ND|8SUB| SUB|[ -]SUEZMAX|([A-Z]|\\s)*((LAST)|(EX\\s)|(OOS)|RPLC|(NO)|(DIRTY)|(N/B)|(WILL)).*$|=.*|
                       \\(.*\\)|<.*>|(\\?)|(^\\+)|\\?|\\(|\\)|FLD|RPLCMNT| CORP| RPL| RPTD|/.*$|FXD|\\?| HOLD| P$|LVL|FAILING|FAILED| OLD| COR$|-.*|CANC$','', test$CHARTERER)
test$CHARTERER <- str_trim(test$CHARTERER)
if(sum(test$CHARTERER == ""|is.na(test$CHARTERER))>0) test$CHARTERER[which(test$CHARTERER == ""|is.na(test$CHARTERER))] <- "CNR"
uncorr <- ""
for (i in 1:nrow(test)){
...
test$RATE <- gsub("E\\+05M", "00K", test$RATE)
test$RATE <- str_trim(test$RATE)
test <- unique(test)
test_c <- test

#### Standardize LAOD_PORT & DISCHARGE_PORT ------------------
...
p_spl <- which(str_detect(test$LOAD_PORT, "/"))
test$DISCHARGE_PORT[p_spl] <- sapply(str_split(test$LOAD_PORT[p_spl], "/"), "[[", 2)
test$LOAD_PORT[p_spl] <- gsub("/.*", "", test$LOAD_PORT[p_spl])
if(sum(is.na(test$LOAD_PORT))>0) test <- test[-which(is.na(test$LOAD_PORT)),]

no_dp <- which((is.na(test$DISCHARGE_PORT)|test$DISCHARGE_PORT=="") & str_detect(test$LOAD_PORT, "XROSS|^X-|^X |^CROSS"))
test$DISCHARGE_PORT[no_dp] <- gsub("XROSS|X-|^X |^CROSS ", "",test$LOAD_PORT[no_dp])
test$DISCHARGE_PORT[is.na(test$DISCHARGE_PORT)] <- "-"
sts_m <- which(str_detect(test$LOAD_PORT, "^STS"))
test$LOAD_PORT <- gsub("^STS ","",test$LOAD_PORT)
...
for (k in c(5,8)){
  ...
  test[,k] <- gsub("\\(.*|\\+1|2P|-S$|-P$|-A$|-R\\.$|-$|\\?","", test[,k])
  ...
  if (sum(test[,k]== "")>0) test[which(test[,k]== ""),k] <- "-"
  nomatch1 <- ""
  ...
  check1 <- which(str_detect(test[,k], "^NW |^EC |^WC |^NORTH |^NC |^CENTRAL |^SOUTH"))
  test[check1, k+1] <- test[check1,k]
  test[check1, k+1] <- gsub("NW |EC |WC |NORTH |NC |CENTRAL |SOUTH |-.*", "", test[check1, k+1])
  test[,k] <- str_trim(test[,k])
  test[,k] <- gsub("-$","",test[,k])
}
nomatch1 <- as.numeric(nomatch1[-1])
nomatch2 <- as.numeric(nomatch2[-1])
test$LOAD_PORT[sts_m] <- paste0("STS ", test$LOAD_PORT[sts_m])
test_c <- test

# Standardize LAYCAN -------------------------
if(sum(is.na(test$LAYCAN))>0) test <- test[-which(is.na(test$LAYCAN)),]
test$LAYCAN <- gsub("ARP", "APR", test$LAYCAN)
test$LAYCAN <- gsub("DED|DDEC", "DEC", test$LAYCAN)
test$LAYCAN <- gsub("JUM|HUN", "JUN", test$LAYCAN)
test$LAYCAN <- gsub("AGU", "AUG", test$LAYCAN)
test$LAYCAN <- gsub("SEPT", "SEP", test$LAYCAN)
test$LAYCAN[which(!str_detect(test$LAYCAN, "END"))] <- gsub("\\?|TH |ST |ND |RD", " ", test$LAYCAN[which(!str_detect(test$LAYCAN, "END"))])
test$LAYCAN <- gsub("\\s+", " ", test$LAYCAN)
test$LAYCAN_END <- ""
test$UPDATE_MONTH <- as.numeric(test$UPDATE_MONTH)
...
for ( i in (1:nrow(test))){
  ...
  t <- 1
  if (str_detect(test$LAYCAN[i], "ELY|MID|END")){
    a <- str_extract(test$LAYCAN[i], "(ELY|MID|END)")
    day <- which(c("ELY","MID","END") == a)*10 - 5 # ELY-5, MID-15, END-26
    test$LAYCAN[i] <- gsub("ELY|MID|END", day, test$LAYCAN[i])
  }
  
  if(str_detect(test$LAYCAN[i], "2018-[0-9]+-[0-9]+")) next
  else if (str_detect(test$LAYCAN[i], "PPT")){
    test$LAYCAN[i] <- test$UPDATE_DATE[i]
    next
  }
  else if (str_detect(test$LAYCAN[i], "[0-9]{1,2}[/ -]?(JAN|FEB|MAR|APR|MAY|JUN|JUL|AUG|SEP|OCT|NOV|DEC)|
                      (JAN|FEB|MAR|APR|MAY|JUN|JUL|AUG|SEP|OCT|NOV|DEC)[/ -][0-9]{1,2}")){
    a <- as.numeric(str_extract(test$LAYCAN[i], "([0-9]{1,2})"))
    b <- str_extract(test$LAYCAN[i], "(JAN|FEB|MAR|APR|MAY|JUN|JUL|AUG|SEP|OCT|NOV|DEC)")
    month <- which(month_abbr == b)
    day <- a
  }
  else if(str_detect(test$LAYCAN[i],"[0-9]+-[0-9]+/[0-9]+")){
    d <- str_extract(test$LAYCAN[i],"[0-9]+/[0-9]+$")
    day2 <- as.numeric(str_extract(d, "[0-9]+"))
    month <- as.numeric(str_extract(str_extract(d, "/[0-9]{1,2}"), "[0-9]{1,2}"))
    day1 <- as.numeric(str_extract(test$LAYCAN[i], "[0-9]+"))
    test$LAYCAN[i] <- paste(year, month, day1, sep="-")
    test$LAYCAN_END[i] <- paste(year, month, day2, sep="-")
    next
  }
  else if (str_detect(test$LAYCAN[i], "^[0-9]{1,2}[ /-][0-9]{1,2}$")){
    a <- as.numeric(str_extract(test$LAYCAN[i], "[0-9]+"))
    b <- as.numeric(str_extract(str_extract(test$LAYCAN[i], "/[0-9]{1,2}"), "[0-9]{1,2}"))
  ...
  if(t == 1) test$LAYCAN[i] <- paste(year, month, day, sep="-")
  else non <- c(non,i)
}
non <- as.numeric(non[-1])
test$LAYCAN[non]
test$LAYCAN_END[which(test$LAYCAN_END== "")] <- test$LAYCAN[which(test$LAYCAN_END== "")]
test$LAYCAN <- as.Date(test$LAYCAN, format = '%Y-%m-%d')
...
test$IMO <- ""
test$DWT <- ""
if(any(is.na(test$VESSEL))) test <- test[-which(is.na(test$VESSEL)),]
for(i in 1:nrow(test)){
  if(str_detect(test$VESSEL[i],"TBN")) {
    ...
    imo_match <- which(imo$name== test$VESSEL[i])
    if(length(imo_match)==0)  test$IMO[i] <- NA
    else if(length(imo_match)==1) {
      if(as.numeric(imo$dwt[imo_match]) > as.numeric(test$CARGO_SIZE[i])*1000){
...
for (i in 1:nrow(test)){
  ...
  else if (str_detect(test$LOAD_PORT[i], "^STS")){
    port <- gsub( "STS ", "", test$LOAD_PORT[i],)
    sts <- 1
  } 
...
k4 <- which(str_detect(test$LOAD_PORT, "^EC NORTH AMERICA|^EC CANADA"))
...
for (i in 1:nrow(test)){
  if(is.na(test$DISCHARGE_PORT[i])|test$DISCHARGE_PORT[i] == "-"){
    ...
    if (str_detect(test$DISCHARGE_PORT[i], "-")){
      port <- str_split(test$DISCHARGE_PORT[i], "-")[[1]][1]
      if(str_detect(test$DISCHARGE_PORT[i], "^EC-WC")){
        test$DISCHARGE_COUNTRY[i] <- str_trim(gsub("EC-WC", "", test$DISCHARGE_PORT[i]))
        port <- paste0(port, " ", test$DISCHARGE_COUNTRY[i])
      } 
    } 
...
k4 <- which(str_detect(test$DISCHARGE_PORT, "^EC NORTH AMERICA|^EC CANADA"))
k5 <- which(str_detect(test$DISCHARGE_PORT, "^WC AFRICA"))
k6 <- which(str_detect(test$DISCHARGE_PORT, ""))
...
for (i in 1:nrow(test)){
  ...
  if(all(!str_detect(cargo_price[1,], cargo_i)|is.na(cargo_price[1,]))) cargo_i <- "CPP"
...
for (i in 1:nrow(test)){
  if (str_detect(test$RATE[i], "^USD[0-9]+(\\.[0-9]+)?")){
    rate1 <-  as.numeric(str_extract(str_extract(test$RATE[i], "USD[0-9]+(\\.[0-9]+)?"), "[0-9]+(\\.[0-9]+)?"))
    ...
    rate1 <-  as.numeric(str_extract(test$RATE[i], "[0-9]+(\\.[0-9]+)?"))
    if(test$LOAD_PORT[i] %in% flat$X1) a <- which(flat$X1== test$LOAD_PORT[i])
    else if(test$LOAD_COUNTRY[i] %in% flat$X1) a <- which(flat$X1== test$LOAD_COUNTRY[i])
    else if(test$LOAD_ZONE[i] %in% flat$X1) a <- which(flat$X1== test$LOAD_ZONE[i])
    else{
      freight_price[i] <- 0 
      next
    } 
...
cargo <- test$CARGO
laycan_start <- test$LAYCAN
laycan_end <- test$LAYCAN
rate <- test$RATE
...
for(i in c(12,16,18,22,26)){
  ...
  if(sum(final[,i]=="-")>0) final[which(final[,i]=="-"),i] <- 0
}
for(i in c(11,13:15,17,19:21)){
  final[,i] <- as.character(final[,i])
  if(sum(final[,i]==0)>0) final[which(final[,i]==0),i] <- "-"
  if(sum(is.na(final[,i]))>0) final[which(is.na(final[,i])),i] <- "-"
  if(sum(final[,i]=="")>0) final[which(final[,i]==""),i] <- "-"
}
...
####Remove exact duplicate with before --------------
cpp_recent1 <- read.csv("~/Desktop/ofe/cpp_recent1.csv", stringsAsFactors = F)
cpp_recent1$laycan_start <- as.Date(cpp_recent1$laycan_start, format = "%Y-%m-%d")
cpp_recent1$laycan_end <- as.Date(cpp_recent1$laycan_end, format = "%Y-%m-%d")
#cpp_recent1$laycan_start <- as.Date(cpp_recent1$laycan_start, format = "%d/%m/%y")
#cpp_recent1$laycan_end <- as.Date(cpp_recent1$laycan_end, format = "%d/%m/%y")
dup_f <- ""
for(i in 1:nrow(final)){
  k <- which(cpp_recent1$vessel_name==final$vessel_name[i]&
               cpp_recent1$charterer==final$charterer[i]&
               cpp_recent1$load_zone_id==final$load_zone_id[i]&
               cpp_recent1$discharge_zone_id==final$discharge_zone_id[i]&
               cpp_recent1$status==final$status[i]&
               abs(cpp_recent1$laycan_start-final$laycan_start[i])<4)
...
write.csv(final_subset, "cpp_clean_27_aug.csv", row.names = FALSE)


####Finalize the fixtures------------
data1 <- read.csv("~/Desktop/ofe/cpp_clean_27_aug_updated.csv", head=TRUE,stringsAsFactors = FALSE)
...
#cpp_recent1$laycan_start <- as.Date(cpp_recent1$laycan_start, format = "%d/%m/%y")
#cpp_recent1$laycan_end <- as.Date(cpp_recent1$laycan_end, format = "%d/%m/%y")
n <- nrow(cpp_recent1)+1
cpp_recent1 <- rbind(cpp_recent1, data1)
cpp_recent1$cargo_size <- as.numeric(cpp_recent1$cargo_size)
time <- format(as.Date(data1$updated_date[1], "%d/%m/%y"), "%y%m%d")
for(i in n:nrow(cpp_recent1)) {
  if (cpp_recent1$charterer[i] %in% char_abbr$char)
    name <-
      char_abbr$name[which(char_abbr$char == cpp_recent1$charterer[i])]
  else name <- "CHAR"
  #time <- format(as.Date(test$UPDATE_DATE[1], format = "%d/%m/%Y"), "%y%m%d")
  char1 <- paste0("T1", name, time)
  same_char <-
    cpp_recent1[which(cpp_recent1$charterer[1:i - 1] == cpp_recent1$charterer[i]), ]
  if (nrow(same_char) == 0) {
    t <- which(str_detect(cpp_recent1$cargo_id[1:i - 1], char1))
    if (length(t) == 0)
      cpp_recent1$cargo_id[i] <- paste0(char1, "001")
    else{
      no <- str_extract(cpp_recent1$cargo_id[t], "[0-9]{2}$") %>% 
        as.numeric() %>% max()+1
      cpp_recent1$cargo_id[i] <- paste0(char1, sprintf('%0.3d', no))
    }
  }
  else if(cpp_recent1$charterer[i] == "CNR") {
    t <- which(str_detect(cpp_recent1$cargo_id[1:i - 1], char1))
    if (length(t) == 0)
      cpp_recent1$cargo_id[i] <- paste0(char1, "001")
    else{
  ...
  else if(cpp_recent1$charterer[i] == "UNIPEC"&cpp_recent1$discharge_port[i] == "CHINA"&
          cpp_recent1$load_port[i] %in% c("AG", "WC AFRICA")){
    k <- which(abs(same_char$laycan_start - cpp_recent1$laycan_start[i]) <= 5 &
                 same_char$vessel_imo == cpp_recent1$vessel_imo[i])
    if(length(k) >0) cpp_recent1$cargo_id[i] <- same_char$cargo_id[k[1]]
    else{
      t <- which(str_detect(cpp_recent1$cargo_id[1:i - 1], char1))
      if (length(t) == 0) cpp_recent1$cargo_id[i] <- paste0(char1, "001")
      else{
        no <- str_extract(cpp_recent1$cargo_id[t], "[0-9]{2}$") %>% 
          as.numeric() %>% max()+1
        cpp_recent1$cargo_id[i] <- paste0(char1, sprintf('%0.3d', no))
    ...
    if (cpp_recent1$cargo_size[i] != 0) {
      k <- which(
        abs(same_char$cargo_size - cpp_recent1$cargo_size[i]) <= 10 &
          same_char$load_zone_id == cpp_recent1$load_zone_id[i] &
          abs(same_char$laycan_start - cpp_recent1$laycan_start[i]) <= 5
      )
      if (length(k) > 0) {
        if (cpp_recent1$vessel_imo[i] %in% same_char$vessel_imo[k]) {
          g <- which(same_char$vessel_imo[k] == cpp_recent1$vessel_imo[i])
          cpp_recent1$cargo_id[i] <- same_char$cargo_id[k[g[1]]]
        }
        else{
          t <- 0
          if (!is.na(cpp_recent1$W[i]) &
              !all(is.na(same_char$W[k]))) {
            g <- which.min(abs(same_char$W[k] - cpp_recent1$W[i]))
            if (abs(same_char$W[k[g[1]]] - cpp_recent1$W[i]) <= 5 &
                cpp_recent1$discharge_zone_id[i] == same_char$discharge_zone_id[k[g[1]]])
              t <- 1
          }
          else if (!is.na(cpp_recent1$USD[i]) &
                   !all(is.na(same_char$USD[k]))) {
            g <- which.min(abs(same_char$USD[k] - cpp_recent1$USD[i]))
            if (cpp_recent1$USD[i] < 10 &
                abs(same_char$USD[k[g[1]]] - cpp_recent1$USD[i]) < 0.1)
              t <- 1
            else if (cpp_recent1$USD[i] > 10 &
                     abs(same_char$USD[k[g[1]]] - cpp_recent1$USD[i]) <= 50)
              t <- 1
          }
          else if (cpp_recent1$discharge_zone_id[i] %in% same_char$discharge_zone_id[k]) {
            g <-
              which(same_char$discharge_zone_id[k] == cpp_recent1$discharge_zone_id[i])
            fix_s <- unique(same_char$cargo_id[k[g]])
            #if(length(fix_s)>1) print(i)
            for (j in 1:length(fix_s)) {
              l <-
                cpp_recent1$laycan_start[which(cpp_recent1$cargo_id[1:i - 1] == fix_s[j])]
              if (abs(max(l - cpp_recent1$laycan_start[i])) <= 5) {
      ...
      if (length(k) > 0) {
        cpp_recent1$cargo_id[i] <- same_char$cargo_id[k[1]]
        cpp_recent1$cargo_size[i] <- same_char$cargo_size[k[1]]
      }
      else{
        t <- which(str_detect(cpp_recent1$cargo_id[1:i - 1], char1))
        if (length(t) == 0)
...
data1 <- data1[,-c(34:35)]
data1$cargo_id <- cpp_recent1$cargo_id[(nrow(cpp_recent1)-nrow(data1)+1):nrow(cpp_recent1)]

#### Match VESSEL TYPE------------
data1$dwt <- as.numeric(data1$dwt)
data1$vessel_type[data1$dwt <= 15000 & data1$dwt != 0] <- "COASTER"
data1$vessel_type[data1$dwt <= 35000 & data1$dwt > 15000] <- "HANDYSIZE"
data1$vessel_type[data1$dwt <= 55000 & data1$dwt > 35000] <- "MR"
data1$vessel_type[data1$dwt <= 80000 & data1$dwt > 55000] <- "LR1"
data1$vessel_type[data1$dwt <= 125000 & data1$dwt > 80000] <- "LR2"
data1$vessel_type[data1$dwt > 125000] <- "SUEZMAX(C)"
...
write.csv(data1, "cpp_27_aug_updated1.csv", row.names = FALSE)

cpp_recent1 <- unique(cpp_recent1)
write.csv(cpp_recent1, "cpp_recent1.csv", row.names= FALSE)
...
write.csv(data_clp, "cpp_27_aug_updated1_clp.csv", row.names = FALSE)
